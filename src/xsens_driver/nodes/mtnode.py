...
class XSensDriver(object):
	...
	def spin_once(self):
		...
		ssAcc_data = data.get('Acceleration') # delta v
		ssGyr_data = data.get('Angular Velocity') # delta q 
		ssMag_data = data.get('Magnetic') # magfield
		ssTime_data = data.get('Timestamp') # all time related data packets
		...
		if ssAcc_data:
			if 'Delta v.x' in ssAcc_data: # found delta-v's
				...
				imu_msg.linear_acceleration.y = ssAcc_data['accY']
				imu_msg.linear_acceleration.z = ssAcc_data['accZ']
				imu_msg.linear_acceleration_covariance = (0., )*9
			else:
				raise MTException("Unsupported message in XDI_AccelerationGroup.")	
					
		if ssGyr_data:
			if 'Delta q0' in ssGyr_data: # found delta-q's
				ss_msg.internal.imu.dq.w = ssGyr_data['Delta q0']
				...
				imu_msg.angular_velocity.y = ssGyr_data['gyrY']
				imu_msg.angular_velocity.z = ssGyr_data['gyrZ']
